...
def mutate_tensor(x, indices, values):
  ...
  for index, val in zip(indices, values):
        i, j = index
        x[i, j] = val
  #khode tensore ro avaz mikone return esh ham mikone
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def slice_indexing_practice(x):
  ...
  - third_col: Tensor of shape (M, 1) giving the third column of x.
    It should be a two-dimensional tensor.9 
...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  idx = torch.arange(x.size(0) - 1, -1, -1)
  y = x[idx, :]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def make_one_hot(x):
  ...
  mask = (torch.tensor(x).unsqueeze(1) == torch.arange(max(x) + 1))
  #az broadcast estefade kardam age mojaz nist mishe ye loop sade ham zad
...
def reshape_practice(x):
  ...
  y = x_reshaped.permute(1, 0, 2).reshape(3, 8)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  y[torch.arange(y.size(0)), min_indices] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
    ...
    for x_batch, y_batch in zip(x, y):
        results.append(torch.mm(x_batch, y_batch))
...
def normalize_columns(x):
  ...
  x_minus_mu = x - mu
  #print(x_minus_mu)
  sum_squares_diff = torch.sum(x_minus_mu ** 2, dim=0)
  #print(sum_squares_diff)
  sigma = torch.sqrt(sum_squares_diff / (x.shape[0] - 1))
  y = x_minus_mu / sigma
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def mm_on_gpu(x, w):
  ...
  #x_gpu = x.cuda()
  #w_gpu = w.cuda()
  #y = torch.matmul(x_gpu, w_gpu).cpu()
  x_gpu = x.cuda()
  w_gpu = w.cuda()
